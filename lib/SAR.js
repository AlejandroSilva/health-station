...
class Sar{
    ...
    discMounted(){
        let command = `sar -F ${this.interval} 1`;
        return new Promise( (resolve, reject)=>{
            // ejecuta el comando...
            Sar._exec(command)
                // si lo ejecuta bien, parsea la respuesta
                .then( (lines)=>{
                    // filtrar las linas que no entrega informacion sobre 'Resumen'
                    let mountValues = lines.map( (line)=>{
                        let lineValues = line.trim().split(/\s+/g);
                        //Resume n      MBfsfree  MBfsused   %fsused  %ufsused     Ifree     Iused    %Iused FILESYSTEM
                        if(lineValues[0]==='Resumen' && lineValues[1]!=='MBfsfree'){
                            return {
                                MBfsfree: +lineValues[1],
                                MBfsused: +lineValues[2],
                                fsused: +lineValues[3].replace(',', '.'),
                                FILESYSTEM: lineValues[8]
                            }
                        }
                    }).filter( (line)=>{
                        return line!==undefined
